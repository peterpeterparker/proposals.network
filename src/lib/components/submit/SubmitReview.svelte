...
{#if nonNullish(html) && nonNullish(neuronId)}
	<p class="leading-relaxed mb-8">Review your proposal for Neuron ID: <mark class="border-2 border-black rounded bg-violet-100 px-1 py-0.5">{neuronId}</mark>. Make sure everything looks good before submitting!</p>

	<div in:fade class="tiptap py-8 px-5 focus:outline-none bg-white border-2 border-black shadow-[2px_2px_0px_rgba(0,0,0,1)] lg:rounded-md overflow-hidden mb-8">
		<Html text={html} />
	</div>
{/if}
